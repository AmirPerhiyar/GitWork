...
int main()
{
    ...
    struct std
    {
        ...
        char roll[20]; /// contact number of Student
        char cell[12]; /// basic salary of Student
    };

    struct std s; /// structure variable creation
   ...
   while(1)
    {
        ...
        case '2':
            ...
            {
                printf("\n %15s \t %15s \t %50s \n",s.name,s.roll, s.cell); /// print the name, age and contact number
            }
            getch();
            break;
        ...
        case '3':  /// if user press 3 then do editing existing record
            ...
            while(another == 'y')
            {
                ...
                scanf("%s", stdname);
                rewind(fp);
                while(fread(&s,recsize,1,fp)==1)  /// fetch all record
                                {
                    if(strcmp(s.name,stdname) == 0)  ///if entered name matches with that in file
                    {
                        printf("\nEnter new name, roll number and cell number: ");
                        scanf("%s%s%s",s.name,s.roll,s.cell);
        ...
        case '4':
            ...
            while(another == 'y')
            {
                ...
                {
                    if(strcmp(s.name,stdname) != 0)  /// if the entered record match
                    {
                ...
                fclose(fp);
                fclose(ft);
                remove("STD.DAT"); /// remove the orginal file
                rename("Temp.dat","STD.DAT"); /// rename the temp file to original file name
                fp = fopen("STD.DAT", "rb+");
